...
import { postData, toastMessageShow, queryOptionData, handleShareholderNameChange, css } from 'service/common.js';
...
const getOppContactDrpDwnList = (e, data) => {
    return queryOptionData(e, "sales/Opportunity/get_contact_list_for_opportunity", { query: e ,limit: ''}, 2, 1);
}
    ...
    componentDidMount() {
        ...
        if (this.props.activeLineItem) {
            ...
            let rows = this.props.activeLineItem.map((contact, i) => {
                    return {
                        id: contact.id,
                        amount: contact.incr_id_opportunity_items > 0? contact.amount : "",
                        ...
                        qty: contact.incr_id_opportunity_items > 0? contact.qty : "",
                        qty_editable: contact.qty_editable,
                        selected: contact.selected,
                        incr_id_opportunity_items: contact.incr_id_opportunity_items,
            ...
            this.getLineItemsAdditionalFundingDetail({opportunity_id: this.props.opp_id});
        }
    }

    getLineItemsAdditionalFundingDetail = (requestData) => {
        
        postData('sales/Opportunity/get_line_items_additional_funding_detail', requestData).then((result) => {
            if (result.status) {
        ...
        this.state.rows.map((row) => {           
            ...
            if(row.selected && row.category_ref === "") {             
               ...
               if(row.amount != "0" && row.amount != "0.00" && row.amount != "") {
                    ...
                    (row.amount != "0" && row.amount != "0.00" && row.amount != "") && 
                        is_parent_amount[row.category_ref] == "0") {
        ...
        return (
            <IconSettings iconPath={"/assets/salesforce-lightning-design-system/assets/icons"}>
                ...
                >
                    ...
                    <form id="lineItemSave" autoComplete="off" className="slds_form" onSubmit={e => e.preventDefault()} disabled={this.props.disabled} style={styles.form}>
