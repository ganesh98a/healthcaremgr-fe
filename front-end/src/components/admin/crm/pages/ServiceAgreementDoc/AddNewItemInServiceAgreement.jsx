...
class AddNewItemInServiceAgreement extends Component {
    constructor(props) {
        super(props);
        checkItsNotLoggedIn();
        this.state = {
    ...
    manageActiveLineItem = () => {
        ...
        if (this.props.callGetLineItem) {
            var requestData = { srch_box: this.state.srch_box, service_agreement_id: this.props.service_agreement_id };
    ...
    getActiveLineItem = (requestData) => {
        this.setState({ loading: true });
        postData('sales/ServiceAgreement/get_finance_active_line_item_listing', requestData).then((result) => {
            if (result.status) {
    ...
    searchData = (e) => {
        ...
        let selected_id = [];
        this.state.activeLineItem.map((data) => {
            if (data.selected) {
                selected_id.push(data.id)
            }
        });

        this.setState({ selected_id: selected_id, alreadySeleactedItem: this.state.selectedLineItems }, () => {
            this.getActiveLineItem(requestData);
        });
    ...
    selectParentLineItem(item_ref, line_item_number) {
        ...
        this.state.activeLineItem.map((item, itemid) => {
            ...
            } else if (item_ref == '' && item.line_item_number === line_item_number && !item.selected) {
                //Disable Parent item during it's deselecte with child item
...
export default AddNewItemInServiceAgreement;