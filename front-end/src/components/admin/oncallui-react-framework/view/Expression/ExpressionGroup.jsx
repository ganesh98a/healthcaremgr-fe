...
import {Combobox, Button, Input} from '@salesforce/design-system-react';
...
class ExpressionGroup extends React.Component {
	...
	static triggerChange(event, data) {
		const selection = data.selection[0] && data.selection[0].id || '';
		let trigger = '';
		if (selection === '1') {
			trigger = 'all';
		} else if (selection === '2') {
		...
		const buttons =
			...
			this.props.triggerType !== 'formula' ? (
				<div className="slds-expression__buttons">
					...
					/>
					{this.props.isRoot && this.props.triggerType !== 'custom' ? (
						<Button
							iconCategory="utility"
							iconName="add"
							iconPosition="left"
