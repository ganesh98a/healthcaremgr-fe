...
class ExpressionCondition extends React.Component {
    ...
    getFieldValue() {
        return this.props.getResourceSelected();
    }
				...
				<fieldset>
					...
					<div className="slds-grid slds-gutters_xx-small">
						<div className="slds-col">
                            <Label text={labels.resource} />
                            <TextBox readOnly onChange={e => this.props.onChangeResource(e, e.target.value) || (() => {return false})} value={this.getFieldValue()} onClick={(e) => this.props.events.onClickResource(e)} />
						</div>
						<div className="slds-col slds-grow-none">
							<SelectList 
                                ...
                                onChange={(value) => this.props.events.onChangeOperator(null, value)}
                                value={this.props.operatorSelected}
                            />
						</div>
						<div className="slds-col  slds-grow-none">
							{!this.props.getValues().length && <Input
								id={`${this.getId()}-input`}
								label={labels.value}
                            ...
                            <SelectList 
                                id={`${this.getId()}-input`}
                                label="Value"
                                options={this.props.getValues()}
                                onChange={(value) => this.props.events.onChangeValue(null, {value})}
                                value={`${this.props.value}`}
                                required
								name="value"
                            />
						...
						<div className="slds-col slds-grow-none">
							<div className="slds-form-element">
								...
								<div className="slds-form-element__control">
									{ this.props.index === 0 && this.props.isGroup === false ? (<div style={{marginRight:"30px"}}></div>) : ( 
									<Button
