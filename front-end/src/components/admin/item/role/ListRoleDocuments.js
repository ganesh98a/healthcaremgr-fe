 ...
 */
const requestData = (role_id, pageSize, page, sorted, filtered) => {
    return new Promise((resolve, reject) => {
        var Arr = { pageSize: pageSize, page: page, sorted: sorted, filtered: filtered };
        var Request = JSON.stringify(Arr);
        console.log(Arr);
        postData('item/document/get_role_documents', { role_id: role_id, request: Request }).then((result) => {
            if (result.status) {
...
class ListRoleDocuments extends Component {
    ...
    constructor(props) {
        ...
        this.state = {
            ...
            selectfilteroptions: [
                { value: "title", label: "Document title", order: "1" },
    ...
    componentDidMount() {
        ...
        this.get_role_details({ roleId: role_id });
    }
    ...
    renderHeader() {
        ...
        const trail = [
			<Link to={ROUTER_PATH + `admin/item/role`} className="default-underlined slds-truncate" style={{ color: '#0070d2' }}>Role</Link>,
			<Link to={ROUTER_PATH + `admin/item/role/details/` + this.state.role_id} className="default-underlined slds-truncate" style={{ color: '#0070d2' }}>{this.state.name}</Link>,
		];
        return (
            <React.Fragment>
                <PageHeader
                    onRenderActions={() => {
                        return (
                            <React.Fragment>
                                <PageHeaderControl>
                                    <Button label="New" onClick={e => this.showAttachDocToRoleModal()} />
                                </PageHeaderControl>
                                </React.Fragment>
                            )
                        }}
...
export default connect(mapStateToProps, mapDispatchtoProps)(ListRoleDocuments);