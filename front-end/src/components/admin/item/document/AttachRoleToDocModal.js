...
class AttachRoleToDocModal extends Component {
    ...
    constructor(props) {
        ...
        this.state = {
            ...
            doc_obj: this.props.doc_details,
    ...
    onSubmit = (e, state) => {
        ...
        if (!this.state.loading && jQuery("#attach_doc").valid()) {
            ...
            var req = {
                ...
                doc_obj: this.state.doc_obj ? this.state.doc_obj.value : ''
            }
            console.log(req);
            postData('item/document/attach_document_and_role', req).then((result) => {
                if (result.status) {
                    ...
                    this.props.hideAttachRoleToDocModal(true);
                } else {
    ...
    componentDidMount() {
        this.setState({ loading: true });
        if (this.props.doc_details.id) {
            this.setState({ doc_id: this.props.doc_details.id });
        }
    ...
    render() {
        return (
            <IconSettings iconPath={"/assets/salesforce-lightning-design-system/assets/icons"}>
                <Modal
                    isOpen={this.props.showAttachRoleToDocModal}
                    footer={[
                        <Button label="Cancel" onClick={this.props.hideAttachRoleToDocModal} />,
                        <Button disabled={this.state.loading} label="Save" variant="brand" onClick={this.onSubmit} />,
                    ]}
                    onRequestClose={this.props.hideAttachRoleToDocModal}
                        ...
                        <div className="row py-1">
                            <div className="col-lg-6 col-sm-6">
                                <div className="slds-form-element">
                                    <label className="slds-form-element__label" htmlFor="select-01"><abbr className="slds-required" title="required">* </abbr>Role</label>
...
export default connect(mapStateToProps, mapDispatchtoProps)(AttachRoleToDocModal);