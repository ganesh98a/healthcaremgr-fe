...
function setParseDate(dateString) {
...
export const SLDSISODatePicker = React.forwardRef(
    ...
    (props, ref) => {
        ...
        if (dateMoment && dateMoment.isValid()) {
            dateValue = dateMoment.format(isoDateTimeFormat)
        }
        ...
        return (
            <div style={{ position: 'relative' }} className={classNames} >
                <DatePicker
                    ...
                    parser={(dateString) => setParseDate(dateString)}
                    formatter={date => date ? moment(date).format(props.format) : ''}
                    onChange={(event, data) => {
                        if (props.onChange) {
                            let dateYmdHis = null
							let dateError = 0;
                            const dtm = moment(data.date.toString())
                            if (dtm.isValid()) {
                                setLastValidDate(dtm)
                                dateYmdHis = dtm.format(isoDateTimeFormat)
                            } else {
								dateError = 1;
							}

                            return props.onChange(dateYmdHis, event, data, dateError, data)
                        }
                    }}
