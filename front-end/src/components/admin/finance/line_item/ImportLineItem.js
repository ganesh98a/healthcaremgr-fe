...
class ImportLineItem extends React.Component {
  constructor(props) {
    super(props);
    this.state = {
      activeCol: '',
  ...
  get_funding_type() {
    ...
    postData('finance/FinanceLineItem/get_finance_line_item_listing_filter_option', requestData).then((result) => {
      if (result.status) {
        this.setState({ funding_type_option: result.data.funding_type_option }, () => { });
      } else {
      }
  ...
  uploadHandler = e => {
    ...
    if (jQuery("#import_line_item_from").valid()) {
      ...
      postImageData("finance/FinanceImportExportLineItem/read_data_from_data_sheet", formData, this).then(responseData => {
        var x = responseData;
                ...
                <form id="import_line_item_from" method="post" autoComplete="off">
                  <div className="row d-flex justify-content-center">
                    <div className="col-lg-6 col-ms-6 col-xs-12">
                        ...
                        <div className="col-lg-6 col-sm-6">
                          ...
                          <span className="required">
                            <DatePicker dateFormat="DD/MM/YYYY" autoComplete={'off'} onChangeRaw={handleDateChangeRaw} placeholderText={"DD/MM/YYYY"} className="text-left"
                              selected={this.state.start_date ? moment(this.state.start_date) : null} name="start_date" onChange={(e) => handleChangeSelectDatepicker(this, e, 'start_date')} required={false}
                              maxDate={(this.state.end_date) ? moment(this.state.end_date) : moment().add(60, 'days')} minDate={moment()} />
                          </span>
                        </div>
                        <div className="col-lg-6 col-sm-6">
                          ...
                          <span className="required">
                            <DatePicker dateFormat="DD/MM/YYYY" autoComplete={'off'} onChangeRaw={handleDateChangeRaw} placeholderText={"DD/MM/YYYY"} className="text-left"
                              selected={this.state.end_date ? moment(this.state.end_date) : null} name="end_date" onChange={(e) => handleChangeSelectDatepicker(this, e, 'end_date')} required={false}
                        ...
                        <div className="col-lg-6 col-sm-6">
                          <span className="required">
                            <Select className="custom_select default_validation"
                              ...
                              inputRenderer={() => <input className="define_input" name={"funding_type"} required={true} readOnly={true} value={this.state.funding_type || ''} />}
                            />
                          </span>
                        </div></div> */}
                      ...
                      {/*<div className="row mt-5 d-flex justify-content-center pb-5 d-none">
                        <div className="col-lg-5">
...
export default connect(mapStateToProps, mapDispatchtoProps)(ImportLineItem);