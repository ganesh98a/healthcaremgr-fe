...
import { rosterFilterOption } from 'dropdown/ScheduleDropdown.js';
...
class NewRequestRoster extends Component {
    constructor(props) {
        super(props);
        this.state = {
            roster: 'roster',
    ...
    searchBox = (key, value) => {
        ...
        state[key] = value;
        this.setState(state, () => {
            var filter = { search_box: this.state.search_box, roster_type: this.state.roster_type, start_date: this.state.start_date, end_date: this.state.end_date, status: 2 }
    ...
    render() {
        ...
        const columns = [
...

            {
                Cell: (props) => <span className="d-flex w-100 justify-content-center align-item-center">
                    <span title={ParticiapntPageIconTitle.par_history_icon} onClick={() => this.setState({ historyId: props.original.id, open_history: true })} className="short_buttons_01">View History</span>
                    <Link title={ParticiapntPageIconTitle.par_view_icon} to={{ pathname: '/admin/schedule/roster_details/' + props.original.id, state: this.props.props.location.pathname }} className="short_buttons_01">View Shift</Link>
                </span>, //Header: <div className="">Action</div>,

                Header: <TotalShowOnTable countData={this.state.total_count} />, headerStyle: { border: "0px solid #fff" }, sortable: false
            },
        ]
        ...
        return (
            <React.Fragment>
                ...
                <SchedulePage pageTypeParms={'new_request'} />
                ...
                <div className="tab-content">
                    ...
                    <div role="tabpanel" className="tab-pane active" id={this.state.active_panel}>
                        ...
                        <div className="row">
                            <div className="col-md-12 Shedule_new_request_roster_Table">
...
export default connect(mapStateToProps, mapDispatchtoProps)(NewRequestRoster);
