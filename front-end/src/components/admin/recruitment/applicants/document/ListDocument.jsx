...
class ListDocument extends Component {
                    ...
                    <div>
                        ...
                        <div className="col-sm-6">
                            <Dropdown
                                ...
                                disabled={(Number(this.props.application_process_status) === 7 || Number(this.props.application_process_status) === 8) ? true : false || Number(this.props.applicant_details.flagged_status) == 2 ? true : false}
                                onSelect={(e) => {
        ...
        return [
            ...
            },
            {
                ...
                <Dropdown
                    ...
                    iconName="down"
                    iconVariant="border-filled"
                    disabled={ Number(props.original.system_gen_flag) === 1 && this.state.validStatus === Number(props.original.document_status)? true : false || Number(this.props.applicant_details.flagged_status) == 2 ? true : false}
                    onSelect={(e) => {
                        if(e.value == 1){ //edit
     ...
     */
     handleOnMarkReference = (status) => {
        ...
        AjaxConfirm({ applicant_id: this.state.applicant_id, application_id: this.state.application_id, mark_as: status }, msg, `recruitment/RecruitmentApplicant/mark_document_status`, { confirm: confirmButton, heading_title: `Verify Mandatory Document Checklist` }).then(result => {
            if (result.status) {
                toastMessageShow(result.msg, "s");
                reFreashReactTable(this, 'fetchDocumentData');
            } else {
