...
class ScheduleInterviewStage extends Component {
    ...
    render() {
        ...
        return (<React.Fragment><div className="time_l_1" >
            ...
            {this.state.openReschedule ? <RescheduleGroupOrCabInterview
                task_stage={3}
    ...
    markAsDeclineStatus = (e, taskId) => {
        ...
        postData('recruitment/RecruitmentTaskAction/task_mark_as_decline_to_applicant', { taskId: taskId, applicant_id: this.props.applicant_id }).then((result) => {
            if (result.status) {
                ...
                let application_id = '';
                let stage_label_id = '';
                // Send application in request if not null
                if (this.props.application && this.props.application.id) {
                    application_id = this.props.application.id;
                }
                // Send stage_label_id in request if not null
                ...
                <div className="time_d_2">
                        ...
                        <div className="Recruit_Time_header bb-1 min-height">
                            ...
                            <div className="Rec_Right_s_1">
                                <div>
                                    ...
                                    <StageStatusDropdown
                                        stage_props={this.props}
                                            disable_status={(this.props.stage_status == 2 && ((this.props.applicant_result == 1 || this.props.applicant_result == 2) || (this.props.quiz_status>0))) ? false : ((this.props.stage_status_details.hasOwnProperty('previous_stage') && this.props.id==this.props.stage_status_details.previous_stage) ? false:true)}
                                            stage_status={this.props.stage_status}
                                            stage_status_option={applicantStageStatus('', this.props.quiz_status, true,{is_recruiter_admin:this.props.is_recruiter_admin,allow_inporgress_status_change:(this.props.stage_status_details.hasOwnProperty('previous_stage') && this.props.id==this.props.stage_status_details.previous_stage) ? 1:0 })}
                                            stageId={this.props.id}
                                            stage_key={this.props.stage_key}
                                            enable_email_template_pop_up={true}
                                            application={this.props.application}
                        ...
                        <div className="row d-flex justify-content-center">
                            <div>
                                ...
                                {(this.props.applicant_result == 0 || !this.props.taskId) ? <span className="O_btn_1 w-big my-1 txt_infor">Pending</span> :
                                    ((this.props.applicant_result == 1) ? <span className="O_btn_1 w-big my-1 txt_success">Applicant Successful</span> :
                                ...
                                <div className="text-center">
                                    {this.props.mark_as_no_show == 1 ? <button disabled={(this.props.status == 1) ? false : true} onClick={() => this.setState({ openReschedule: true })} className="limt_btns">Reschedule</button>
                                        : <Link disabled={this.props.quiz_status>0? false: true} to={'/admin/recruitment/group_interview_result/' + this.props.applicant_id + '/' + this.props.taskId} className="under_l_tx">View Results</Link>}
                                </div>
                            </div>
                        </div>
                    ...
                    {this.state.openReschedule ?
                        <RescheduleGroupOrCabInterview
                            task_stage={3} showModal={this.state.openReschedule}
                            closeModal={this.closeRescheduleModel}
                            applicant_id={this.props.applicant_id}
                            request_type={"reschedule_task"} F
                        /> : ''}
