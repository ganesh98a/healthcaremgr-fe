...
export const ApplicationCreateOrUpdateMeetingInvite = (props) => {
    ...
    const checkUserLoggedIn = (e) => {
        ...
        e.preventDefault();
        if (props.validate_added_applicants()) {
    ...
    const createOrUpdateMeetingInvite = () => {
            ...
            callMsGraphForMeeting(res.accessToken, request_data).then(response => {
                if (!response.error && response.id) {
                    ...
                    if (response.error && response.error.code == 'ErrorItemNotFound') {
                        toastMessageShow(response.error.message + 'Please login by valid organizer', "e");
                    }
                }
                ...
                setDisableButton(false);
                console.log(err);
                save_ms_error_log(err, 'Add attendees / generate meeting link', 'Application Listing');
                localStorage.removeItem("ms_username");
                localStorage.removeItem("ms_name");
                localStorage.removeItem("ms_token");
            })
        ...
        const updateMeetingLink = (res, request_data, selected_applicants) => {
            callMsGraphUpdateMeeting(res.accessToken, request_data, props.event_id).then(response => {
                ...
                setDisableButton(false);
                console.log(err);
                save_ms_error_log(err, 'Add extra attendees / update meeting link', 'Application Listing');
                localStorage.removeItem("ms_username");
                localStorage.removeItem("ms_name");
                localStorage.removeItem("ms_token");
            })
    ...
    return (
        <div>
            <Button label="Later" key={1} variant="brand" onClick={() => props.closeModal(true)} />
